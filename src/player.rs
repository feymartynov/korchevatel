...
use crate::character::{AttackInput, Registry as CharacterRegistry};
...
fn control(
    ...
    mut q: Query<(Entity, &mut MovementInput, &mut AttackInput), With<Player>>,
    ...
    let Ok((player, mut movement_input, mut attack_input)) = q.single_mut() else {
        return;
    };
    ...
    // Атака
    *attack_input = if keyboard_input.pressed(KeyCode::Space) {
        AttackInput::Attack
    } else {
        AttackInput::Idle
    };
